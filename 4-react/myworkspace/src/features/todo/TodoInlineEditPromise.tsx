...
const Todo = () => {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:8080/todos")
      // fecth 함수를 실행하고 네트워크 통신이 완료되면 then에 있는 함수(callback)를 실행함
      // then에 있는 callback 함수의 매개변수로 처리결과를 넘겨줌
      ...
      .then((data: TodoItemReponse[]) => {
        ...
        // 로딩중 표시까지
        // setTimeout(() => {
        //   setLoading(false); // 로딩중여부  state 업데이트
        //   setTodoList(todos); // todo state 업데이트
        // }, 300);

        // const todos = data as TodoItemState[]; // 타입 단언(Type Assertions)
        // console.log(todos);
        // console.log(todos[0].createdTime);
      });
